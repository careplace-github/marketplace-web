...
export default function NavList({ item }: { item: NavItemBaseProps }) {
  ...
  const mainList = children ? children.filter((list) => list.type !== 'Common') : [];

  const commonList = children ? children.find((list) => list.type === 'Common') : null;
